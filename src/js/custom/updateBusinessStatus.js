document.addEventListener('DOMContentLoaded', () => {
  ...
  function getScheduleForDay(day) {
    // day: 0 Sun ... 6 Sat
    if (day >= 1 && day <= 5) return hoursWeek; // Mon-Fri
    if (day === 6) return hoursSat;             // Sat
    return null;                                // Sun closed
  }
  ...
  function getNextOpenHint() {
    ...
    // Если сегодня рабочий день (не вс)
    if (schToday) {
      const startM = schToday.start * 60;
      const endM = schToday.end * 60;

      // До открытия -> сегодня в start
      if (mins < startM) {
        return t.todayAt(formatTimeHHMM(schToday.start));
      }

      // После закрытия -> ищем следующий рабочий день
      if (mins >= endM) {
        const tomorrow = (day + 1) % 7;
        const schTomorrow = getScheduleForDay(tomorrow);

        // Завтра рабочий (пн-сб)
        if (schTomorrow) {
          // Если завтра понедельник/вторник/.../суббота — можно "завтра в ..."
          return t.tomorrowAt(formatTimeHHMM(schTomorrow.start));
        }

        // Завтра воскресенье -> понедельник 08:00
        return t.mondayAt(formatTimeHHMM(hoursWeek.start));
      }

      // Иначе мы внутри рабочего времени — подсказка не нужна
      return '';
    }

    // Сегодня воскресенье -> понедельник 08:00
    return t.mondayAt(formatTimeHHMM(hoursWeek.start));
  }
